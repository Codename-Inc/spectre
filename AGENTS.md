...
- `plugins/caspar/` is the canonical plugin source.
- `plugins/caspar-codex/` is generated Codex output. Do not hand-edit it.
- After changing canonical skills, agents, hooks, or plugin assets that ship to Codex, run:
  ...
  ```
- If generated output is wrong, fix the canonical source or the translator under `scripts/`; do not patch `plugins/caspar-codex/` directly.

## Change Workflow

1. Check `git status --short --branch` before editing.
2. Identify the change type:
   - workflow skill: edit `plugins/caspar/skills/<skill>/SKILL.md`
   - subagent: edit `plugins/caspar/agents/*.md`
   - learning/runtime helper: edit `plugins/caspar/hooks/scripts/*`
   - Codex install/update behavior: edit `src/` and load the project skill `feature-codex-caspar-implementation` first
...
7. Review the diff. Generated `plugins/caspar-codex/` changes are expected after sync; manual edits there are not.
...
- For skill rewrites or compression, use the `caspar-rewrite-skill` guidance as the quality bar before editing.
