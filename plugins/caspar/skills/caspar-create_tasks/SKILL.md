...
- If requirements are too thin to extract scope → ask, or route to `/caspar:plan`; do not invent scope.
...
- Research only if plan/context do not name target files/patterns: dispatch `@caspar:finder`, `@caspar:analyst`, `@caspar:patterns`; fold returns into `task_context.md` `## Technical Research`. Skip for LIGHT or clear single-component scope.
...
- No requirements artifact and thread context is too thin → stop; route to `/caspar:scope` or `/caspar:plan`.
