...
- `caspar-test` work completed by `@caspar:tester` test-lead subagents using the primary's risk plan.
...
4. **Test phase.** Dispatch `@caspar:tester` test-lead subagents in parallel. Each subagent loads `Skill(caspar-test)`, consumes the primary risk plan for its batch, and does the test/verification work. P0 gets dedicated focus; P1/P2 may be grouped; P3 is skipped with reason.
