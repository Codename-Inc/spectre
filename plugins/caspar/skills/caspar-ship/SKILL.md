...
## Working Set
- The code area named in `RELEVANT_FILES`/`SCOPE_SUMMARY` (locate via `@caspar:finder`, understand via `@caspar:analyst`).
...
- A pushed branch + a PR (`gh pr create`) with title `{INTENT_TYPE}({scope}): {SCOPE_SUMMARY}` (<70 chars) and body sections: Summary · Changes · Test Plan · "Shipped autonomously via `/caspar:ship`".
...
2. **Quick scope** — fan out `@caspar:finder` + `@caspar:analyst` (read-only), write the ~20-line scope.md. This is a light scope, not full `/caspar:scope`.
...
4. **Execute (TDD)** — per task sequentially: `TaskUpdate`→in_progress; load `Skill(caspar-tdd)`; **RED (failing test first) → GREEN (minimal impl) → REFACTOR**; commit; `TaskUpdate`→completed. Skip TDD only for config/doc-only tasks. If new work surfaces, create a new task — do not scope-creep the current one.
