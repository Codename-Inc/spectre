...
|---|---|---|
| Coverage | `@caspar:analyst` | every plan Verification signal maps to a `test`/`observable`/`state` AC; every in-scope requirement is represented; no Out-of-Bounds task exists |
| Executability | `@caspar:reviewer` | RED pairing for behavior changes, executable AC wording, subtask size cap, no mid-task scope judgment |
| Integration graph | `@caspar:patterns` | predecessor/unblocks, produces/consumed_by/replaces, Phase 0 dependency verification, no orphaned outputs |
| Index alignment | `@caspar:finder` | `execute.md` manifest/source/summary/waves/parent index match `tasks.json`; referenced files and context anchors exist |
