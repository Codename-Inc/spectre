...
3. **Research in parallel**, read-only — locator → analyzer-on-findings → breadth. Spawn follow-ups if a thread is shallow. Use Context7 MCP for central 3rd-party libs.
   ...
   |---|---|---|
   | `@caspar:finder` | relevant files, entry points, handlers, models | file paths by domain |
   | `@caspar:analyst` | data flow, dependencies, behavior, edge cases | file:line for ALL findings |
   | `@caspar:patterns` | similar impls, patterns to follow/avoid | code examples w/ file:line |
   | `@caspar:web-research` | best practices, prior art, pitfalls | findings WITH links |
