...
- `@caspar:analyst` for parallel hypothesis investigation.
...
2. **Investigate in parallel.** Dispatch one `@caspar:analyst` per top hypothesis; each traces its hypothesis through the codebase (relevant code, recent commits, similar patterns, edge cases) and returns a compressed (~1–2K) finding in-thread — no scratch files.
...
4. **Implement test-first.** Per `Skill(caspar-tdd)`: write the failing test, confirm RED, then implement the fix plus the `[🪳 TEMP {TOPIC}]` logging to GREEN. Address the root cause; do not paper over the symptom.

## Handoff
Summarize what you delivered and the concrete validation steps. Then render the inline Next Steps line pointing to the natural follow-on (e.g. `/caspar:test` or `/caspar:clean`).
