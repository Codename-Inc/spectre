...
- **Registry** (read first): `{{project_root}}/.claude/skills/caspar-recall/references/registry.toon` — lines `{skill-name}|{category}|{triggers}|{description}`.

## Working Set
- Skill files: `{{project_root}}/.claude/skills/{category}-{slug}/SKILL.md`.
- Recall skill + registry: `{{project_root}}/.claude/skills/caspar-recall/`.

## Method / guardrails
1. **Context or investigate.** If the topic was discussed in detail or you already understand it this session, proceed. Otherwise enter **Investigation Mode** — do NOT fabricate. Dispatch `@caspar:finder` to map relevant files, then 2–3 read-only `@caspar:analyst` passes answering the category's required-section questions (cite file:line). Synthesize: cross-reference shared files, resolve conflicts by reading the disputed code, flag gaps.
