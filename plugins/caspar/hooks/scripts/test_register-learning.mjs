 ...
 *
 * Run with: node --test plugins/caspar/hooks/scripts/test_register-learning.mjs
 */
...
function createTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'caspar-rl-'));
}
...
describe('register_learning', () => {
  it('creates new registry with entry', () => {
    ...
    try {
      ...
      const registryPath = path.join(tmp, '.claude', 'skills', 'caspar-recall', 'references', 'registry.toon');
      assert.ok(fs.existsSync(registryPath));

      const content = fs.readFileSync(registryPath, 'utf8');
      assert.ok(content.includes('# CASPAR Knowledge Registry'));
  ...
  it('generates recall skill with template', () => {
    ...
    // Create template
    const templateDir = path.join(pluginRoot, 'skills', 'caspar-learn', 'references');
    ...
    try {
      ...
      const skillPath = path.join(tmp, '.claude', 'skills', 'caspar-recall', 'SKILL.md');
  ...
  it('resolves recall template from Codex home when hooks are under caspar runtime', () => {
    const tmp = createTmpDir();
    const codexHome = path.join(tmp, 'codex-home');
    const runtimeRoot = path.join(codexHome, 'caspar');

    const templateDir = path.join(codexHome, 'skills', 'caspar-learn', 'references');
    fs.mkdirSync(templateDir, { recursive: true });
    fs.writeFileSync(
      path.join(templateDir, 'recall-template.md'),
      '# Codex Recall Skill\n\nRegistry:\n{{REGISTRY}}\n\nEnd.\n'
...
describe('register_learning - trigger migration', () => {
  ...
  it('migrates all registry entries on any registration', () => {
    ...
    try {
      ...
      const registryDir = path.join(tmp, '.claude', 'skills', 'caspar-recall', 'references');
