...
- **Action** — SummarizeAndTransition:
  ...
  >
  > **Options**:
  > 1. "Proceed with scope" → `/spectrl:scope` with FROM_KICKOFF=true, SKIP_EXPLORATION=true
  > 2. "Skip to planning" → `/spectrl:plan` with kickoff doc context
  > 3. "Need to think" → `/spectrl:handoff`

- **Action** — ExecuteChoice:
  - **If** scope → invoke `/spectrl:scope` with: FROM_KICKOFF=true, KICKOFF_DOC={path}, SKIP_EXPLORATION=true, context summary
  - **If** planning → suggest `/spectrl:plan`
  - **If** pause → run `/spectrl:handoff`
