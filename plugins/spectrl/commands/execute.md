...
- **Action** — ExecuteAdaptiveLoop: Until all tasks complete:
  ...
  2. **Dispatch Wave**: Launch parallel @dev subagents (1 per task batch)
     ...
     - Instruct: "Read scope docs first to understand E2E UX and integration points. Load @skill-spectrl:spectrl-tdd, then execute tasks sequentially using its TDD methodology. **Commit after each parent task** with conventional commit format (e.g., `feat(module): add X`, `fix(module): resolve Y`). Return completion report with **Implementation Insights** + **E2E Completeness Check**."
...
- **Action** — ExecutedeveviewLoop: Until no critical/high feedback:
  ...
  1. **Spawn Review**: @dev subagent runs `/spectrl:code_review`
...
- **Action** — SpawnValidation: @reviewer runs `/spectrl:validate` with task list
- **Action** — AddressGaps: If high priority gaps → dispatch @dev subagents to fix

## Step 4 - Prepare for QA

- **Action** — GenerateTestGuide: @dev runs `/spectrl:create_test_guide`
