...
> **⚠️ NON-NEGOTIABLE**: This workflow MUST invoke slash commands via the Skill tool. Failure to invoke `/spectrl:create_plan` and `/spectrl:create_tasks` is a critical error. Do NOT summarize, describe, or skip these commands. INVOKE THEM.
...
- Use the **Skill** tool with `skill: "spectrl:create_plan"` and `args: "{path} --depth {tier}"` — generates plan.md
- Use the **Skill** tool with `skill: "spectrl:create_tasks"` and `args: "{path}"` — generates tasks.md
...
│  ❌ WRONG: "The next step would be to run /spectrl:create_plan"        │
│  ❌ WRONG: Ending turn without invoking Skill tool                     │
│                                                                        │
│  ✅ CORRECT: Skill tool with skill: "spectrl:create_plan", args: "..." │
│  ✅ CORRECT: Skill tool with skill: "spectrl:create_tasks", args: "..."│
└────────────────────────────────────────────────────────────────────────┘
```
...
- Use the Skill tool: `skill: "spectrl:create_plan"`, `args: "{OUT_DIR}/task_context.md --depth {tier}"`
- Use the Skill tool: `skill: "spectrl:create_tasks"`, `args: "{OUT_DIR}/task_context.md"`
...
- **If LIGHT**:
  ...
  - **INVOKE NOW** → Skill tool with `skill: "spectrl:create_tasks"`, `args: "{OUT_DIR}/task_context.md --depth light"`
  - **Wait** — Returns task breakdown with brief implementation approach
  - Skip to footer

- **ElseIf STANDARD**:

  - **INVOKE NOW** → Skill tool with `skill: "spectrl:create_plan"`, `args: "{OUT_DIR}/task_context.md --depth standard"`
  - **Wait** — Returns focused plan (Overview, Approach, Out of Scope)
  - **Action** — PromptUser: "Review plan. Reply 'Approved' or provide feedback."
  - **Wait** — User approval
  - **INVOKE NOW** → Skill tool with `skill: "spectrl:create_tasks"`, `args: "{OUT_DIR}/task_context.md"`
  - **Wait** — Returns task breakdown

- **ElseIf COMPREHENSIVE**:

  - **INVOKE NOW** → Skill tool with `skill: "spectrl:create_plan"`, `args: "{OUT_DIR}/task_context.md --depth comprehensive"`
...
- **Action** — RenderFooter: Use `@skill-spectrl:spectrl-next-steps` skill for Next Steps