...
- **What** — Triage changes into risk tiers, dispatch @spectrl:tester subagents to fix lint and write **risk-appropriate tests** (not brute-force 100% line coverage), and commit after each passing batch.
...
- **Action** — DispatchTestWriter: Spawn MULTIPLE @spectrl:tester subagents IN PARALLEL

  - **Parallelization Strategy**:
    - Partition test plan items into independent batches (by file or logical grouping)
    - Dispatch one @spectrl:tester per batch — aim for 3-5 parallel agents for medium scope, up to 8 for large scope
...
- **Action** — RunTests: Execute full test suite
  ...
  - **If** tests fail → analyze failure, fix via @spectrl:tester or direct edit
  - **Else** → continue

- **Action** — VerifyQuality: Spot-check 1-2 tests

  - Confirm: tests assert behaviors, would catch real bugs, survive refactoring
  - **If** test quality poor → rework via @@spectrl:tester
  - **Else** → continue
...
- **Action** — RenderFooter: Render Next Steps footer using `@skill-spectrl:spectrl-next-steps` skill (contains format template and spectrl command options)

## Next Steps

See `@skill-spectrl:spectrl-next-steps` skill for footer format and command options.
...
- [ ] Multiple @spectrl:tester agents dispatched in parallel (not sequential)
