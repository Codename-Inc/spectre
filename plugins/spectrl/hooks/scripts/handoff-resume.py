...
SessionStart hook that injects context from the last /spectrl:handoff.
...
def copy_plugin_references():
    """Copy plugin reference files to .claude/spectrl/ for command access.
    ...
    Also appends .claude/spectrl/ to .gitignore if it exists and .claude/ not already ignored.
    """
    ...
    references_dst = Path(".claude/spectrl")
    ...
    if gitignore.exists():
        ...
        if ".claude/" not in content and ".claude/spectrl/" not in content:
            with open(gitignore, "a") as f:
                f.write("\n# spectrl plugin files\n.claude/spectrl/\n")
...
def format_context(
    ...
    notice_lines.append("\n💡 Run /spectrl:forget to clear session memory and start fresh.")
