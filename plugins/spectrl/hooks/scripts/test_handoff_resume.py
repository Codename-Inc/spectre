...
class TestCopyPluginReferences:
    ...
    def test_copies_md_files_from_plugin_references_to_claude_spectrl(self, tmp_path, monkeypatch):
        """Happy path: copies .md files from CLAUDE_PLUGIN_ROOT/references/ to .claude/spectrl/."""
        ...
        # Verify
        spectrl_dir = project_dir / ".claude" / "spectrl"
        assert spectrl_dir.exists()
        assert (spectrl_dir / "next_steps_guide.md").exists()
        assert (spectrl_dir / "other_reference.md").exists()
        assert (spectrl_dir / "next_steps_guide.md").read_text() == "# Next Steps Guide\nContent here"
    ...
    def test_returns_early_when_plugin_root_not_set(self, tmp_path, monkeypatch):
        ...
        # Verify - .claude/spectrl should NOT be created
        spectrl_dir = project_dir / ".claude" / "spectrl"
        assert not spectrl_dir.exists()

    def test_appends_to_gitignore_when_exists_and_claude_not_ignored(self, tmp_path, monkeypatch):
        """Appends .claude/spectrl/ to .gitignore when it exists and .claude/ not already ignored."""
        sys.path.insert(0, str(SCRIPT_PATH.parent))
        from importlib import import_module
        handoff_resume = import_module("handoff-resume")
        ...
        assert ".claude/spectrl/" in content
