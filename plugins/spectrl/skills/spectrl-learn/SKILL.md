...
```
{{project_root}}/.claude/skills/spectrl-find/references/registry.toon
```

Format: `{skill-name}|{category}|{triggers}|{description}` (one learning per line)

Example: `feature-spectrl-plugin|feature|spectrl, /learn, /find|Use when modifying spectrl plugin or debugging hooks`
...
This updates the registry and regenerates the find skill at `.claude/skills/spectrl-find/`.
...
**Good descriptions:**
- `"Use when modifying spectrl plugin, debugging hooks, or adding knowledge categories"`
...
**Bad descriptions:**
- `"spectrl plugin architecture - how knowledge capture works"` (describes content, not when to use)
