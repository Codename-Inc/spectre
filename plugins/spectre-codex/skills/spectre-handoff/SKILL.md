...
Spawn `@spectre_sync` subagent with the composed data and the literal paths returned by Step 1:
...
- Pass the resolved history directory and canonical output file as literal paths to `@spectre_sync`. Neither skill nor agent reconstructs branch identity from those paths.
