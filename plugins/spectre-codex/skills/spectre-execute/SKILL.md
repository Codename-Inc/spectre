...
- **Action** — ExecutedeveviewLoop: Until no critical/high feedback:
  ...
  1. **Spawn Review**: @dev subagent runs `Skill(spectre-code_review)` (Claude slash route: `spectre-code_review`)
...
- **Action** — SpawnValidation: @reviewer runs `Skill(spectre-validate)` (Claude slash route: `spectre-validate`) with task list
