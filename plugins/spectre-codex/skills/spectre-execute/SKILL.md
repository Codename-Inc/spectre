...
- High-priority review or validation gaps → dispatch `@spectre_dev` to fix, rerun deterministic checks, then rerun `Skill(spectre-code_review)` or only the affected validation check.
