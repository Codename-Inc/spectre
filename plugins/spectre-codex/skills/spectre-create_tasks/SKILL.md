...
- When plan/context omit target files or patterns, use read-only `@spectre_finder`, `@spectre_analyst`, and `@spectre_patterns` as needed; skip for LIGHT or clear single-component work. Agents return <=2,000-token evidence; the primary folds accepted evidence into `task_context.md`.
