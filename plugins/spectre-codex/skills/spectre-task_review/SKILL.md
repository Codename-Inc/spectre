...
10. The reviewer writes the complete Findings section before editing `TASKS_JSON`, applies only authorized scope-safe changes, appends one disposition and resulting task edit per finding, and then finishes `REVIEW_REPORT`. Run `task-review-safety.mjs` `validate-report` against those outputs and the protected hashes in `PREFLIGHT_JSON`:
   ...
   ```
11. After a completed external route, a missing or mechanically invalid report gets one same-route report-only repair attempt that may edit only `REVIEW_REPORT`, then validation runs again. The primary does not recreate or semantically reconfirm reviewer output, including by correcting semantic content itself. If the report remains unusable, use native fallback under the same round; a failed native fallback leaves the round `incomplete` and never replays the terminal external route.
...
15. The primary owns deterministic validation and ledger transitions only. It does not recreate or semantically reconfirm findings, apply semantic edits, or modify `TASKS_JSON`/`REVIEW_REPORT` after a completed route.
...
The closed consumer-safety report gate requires a confined/readable report, a parseable Findings table or explicit no-findings form, location/finding/edit fields, one valid disposition per finding, required route metadata, and unchanged protected scope/plan hashes. Missing Coverage Summary requests report-only repair or becomes an advisory; it does not silently expand the helper's hard-failure set.
