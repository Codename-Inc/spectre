...
- **LIGHT** → `Skill(spectre-create_plan)` `{OUT_DIR}/task_context.md --depth light --no-review` → `Skill(spectre-create_tasks)` `… --depth light`. Verify `execute.md` sections + `tasks.json` parse; summarize artifacts; state LIGHT skipped review + human gate.
...
- **STANDARD** → `Skill(spectre-create_plan)` `{OUT_DIR}/task_context.md --depth standard --no-review` → `Skill(spectre-plan_review)` `{OUT_DIR} --mode adversarial --auto-apply scope-safe` → `Skill(spectre-create_tasks)` `{OUT_DIR} --depth standard`.
- **COMPREHENSIVE** → `Skill(spectre-create_plan)` `{OUT_DIR}/task_context.md --depth comprehensive --no-review` → `Skill(spectre-plan_review)` `{OUT_DIR} --mode full --auto-apply scope-safe` → `Skill(spectre-create_tasks)` `{OUT_DIR} --depth comprehensive` → `Skill(spectre-task_review)` `{OUT_DIR} --mode adversarial --auto-apply scope-safe`.
