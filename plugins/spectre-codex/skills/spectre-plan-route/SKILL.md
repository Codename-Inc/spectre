...
- If evidence is the only reason to cross a materially costlier route, use exactly one bounded classification probe with `@spectre_finder` or `@spectre_patterns` to answer one repository question.
