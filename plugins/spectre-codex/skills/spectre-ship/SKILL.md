...
- Reuse `FEATURE_ROOT` only with explicit/current-thread evidence (physical directory wins; never branch/recency/lifecycle scans). Otherwise standalone loads `Skill(spectre-feature-root)` through DONE; orchestrated calls escalate. Keep writes beneath it.
