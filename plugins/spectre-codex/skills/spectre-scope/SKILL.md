...
- **Ground once.** Run exactly **one** fast lookup to anchor the hypothesis in repo reality — a single `@finder` query, or one `grep`/`glob`. One call only; skip it if slow. Needing more than one signal means this is bigger than a scope — gather context more fully before continuing rather than forcing it here.
