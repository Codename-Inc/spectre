...
- **Ground once.** Start with exactly **one** fast lookup to anchor the hypothesis in repo reality — a single `@spectre_finder` query, or one `grep`/`glob`; skip it if slow. In default interactive mode, needing broader grounding exceeds this fast scope pass. One-confirmation mode explicitly exempts the targeted read-only pre-research below from the one-lookup limit.
...
- **One-confirmation delivery mode.** When `DELIVERY_ALIGNMENT=one-confirmation`:
  1. If the fast lookup is insufficient, run targeted read-only `@spectre_finder`, `@spectre_analyst`, and `@spectre_patterns` pre-research before prompting.
