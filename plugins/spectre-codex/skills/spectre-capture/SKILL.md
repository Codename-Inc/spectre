...
4. To revise, exact-load `knowledge-cli.mjs load <id> --json`; use its `record` schema and `revisionToken` to materialize `<proposal-root>/<exact-id>/record.json` outside the knowledge store. Edit and register only that proposal with `--expected-revision`; never edit canonical packages, `index.json`, or history, and preserve failed proposals. Reconcile stale evidence by reloading and comparing it; surface contradictions with an explicit user decision. Already-authorized accepted facts need no new proposal or approval loop.
5. For work, resolve or carry the stable ID from an exact run, PR, or candidate association—never branch or recency. Keep execution, verification, and PR state separate; `work.pullRequest.state: draft-open` never implies merged. Knowledge may retain source run references until the work account links them without rewriting it.
