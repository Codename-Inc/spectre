...
3. If the opposite CLI is missing, exits non-zero, cannot write `REVIEW_REPORT`, or produces no semantically usable review after primary-owned report normalization, record the reason and fall back to one native `@spectre_reviewer`; unavailable opposing runtimes never block completion.
