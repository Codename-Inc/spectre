...
After any user conversation, re-orient: if STANDARD/COMPREHENSIVE and the Initial Design Proposal Gate is not approved, stay in step 3; if approved and `## Selected Design` exists in `task_context.md`, continue to step 4; only then execute the next required Skill invocation. **YOU MUST** call these via the Skill tool. The primary authors plan/task/index/goal artifacts under the loaded contracts except for the explicit scope-safe reviewer write surfaces in `plan_review` and `task_review`; it never self-authors or semantically reconfirms independent review findings, or ends the turn saying "I'll now…".
