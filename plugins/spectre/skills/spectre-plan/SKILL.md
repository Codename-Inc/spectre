...
- **Independent plan-review pipeline (STANDARD-family/COMPREHENSIVE):** one `spectre-plan_review` invocation runs two fresh high-effort opposing-runtime stages over one shared bounded evidence wave: correctness writes `reviews/plan_correctness*.md` and applies authorized scope-safe corrections first; simplification then writes `reviews/plan_review*.md` and removes only complexity not protected by correctness. The primary dispatches read-only evidence agents and validates protected inputs/outputs but **MUST NOT** substitute findings or reconstruct semantic writeback. Both stages write findings before `plan.md` edits, record every disposition/resulting edit, and withhold Scope-Change-Required findings.
...
- **Single automatic plan-review pipeline:** each planning run invokes `spectre-plan_review` at most once, before task generation (structured) or goal generation (direct); that invocation owns exactly one correctness stage followed by one simplification stage. Later artifact edits or scope-preserving feedback never trigger another pipeline. A new explicit user request invokes it normally; the planner never infers another review.
- **Review routing:** a pipeline missing either valid paired report, or returning unresolved correctness Blocker/High, stops before `spectre-create_tasks` or later skills. `spectre-task_review` retains its own completed-round and recovery contract.
...
- STANDARD-DIRECT: LIGHT outputs + paired saved correctness/simplification reports under `{FEATURE_ROOT}/reviews/`, with scope-safe plan findings integrated before goal generation.
- STANDARD: `{FEATURE_ROOT}/specs/plan.md` + `specs/execute.md` + `specs/tasks.json` + `goal-prompts.md` + paired saved correctness/simplification reports under `{FEATURE_ROOT}/reviews/`, with scope-safe plan findings integrated before task generation.
