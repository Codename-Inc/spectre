...
**2. Research by tier.** MICRO/LIGHT: cheap scan only (LIGHT may dispatch `@spectre:finder` if files unclear). STANDARD: ≤2 of `@spectre:finder`/`@spectre:analyst`/`@spectre:patterns`; no `@spectre:web-research` unless a new external dep/API/framework is likely. COMPREHENSIVE: all needed dimensions incl. `@spectre:web-research` only when external choices matter. For non-MICRO, write findings to `{OUT_DIR}/task_context.md` `## Technical Research` so downstream skills consume instead of re-research.
