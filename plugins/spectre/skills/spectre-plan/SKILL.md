...
Planning router: classify tier, spend a tier-sized research budget, then route to the smallest safe workflow. **The primary planning agent owns synthesis and directly writes every planning artifact**; research agents return evidence and independent reviewers return reports. Skill invocation loads procedure into that same primary; it does not transfer ownership.
...
- Estimate policy: for STANDARD/COMPREHENSIVE gates, read `references/estimation-guidance.md` at runtime and prefer its optional local telemetry snapshot over the shipped seed prior. Never scan transcripts during a live Plan.
...
Immediately before the approval sentence, render the compact **Historical guidance** block required by `references/estimation-guidance.md`. Guidance is informational, excludes user-response waits, and never delays or blocks the gate; render its unavailable state when no valid estimate exists.
...
After any user conversation, re-orient: if STANDARD/COMPREHENSIVE and the Initial Design Proposal Gate is not approved, stay in step 3; if approved and `## Selected Design` exists in `task_context.md`, continue to step 4; only then execute the next required Skill invocation. **YOU MUST** call these via the Skill tool, then the same primary directly authors or updates `plan.md`, `execute.md`, `tasks.json`, and `goal-prompts.md` under the loaded contracts. Never delegate those writes, self-author independent review findings, or end the turn saying "I'll now…".
...
- **Estimate semantics:** follow `references/estimation-guidance.md`; never present historical guidance as a commitment, invoice, or subscription charge. Dollar guidance is directional API-equivalent for subscription-authenticated Claude/Codex usage; for direct API usage it represents estimated API spend only when the provider/model, rate basis, and pricing date are stated.
- **Independent plan_review (STANDARD/COMPREHENSIVE):** `spectre-plan_review` must produce a saved plan-only review through an opposite-runtime CLI reviewer when available, or through explicit same-runtime Spectre subagent fallback when not. The reviewer writes only the report; the primary **MUST NOT** substitute self-authored findings. The primary reads the report and directly edits `plan.md` so every scope-safe Blocker/High is reflected before task generation; never dispatch a subagent or external reviewer to apply findings. **Do not apply Scope-Change-Required findings** — surface them.
- **Task artifact review (COMPREHENSIVE only):** `spectre-task_review` checks only generated `execute.md` + `tasks.json` against the reviewed plan. The primary directly applies scope-safe Blocker/High task fixes to `tasks.json` and affected execute-index rows; never dispatch write-back, and never edit `plan.md` or scope docs from task review.
- **Single automatic plan review:** each planning run invokes `spectre-plan_review` at most once before task generation. Later artifact edits or scope-preserving feedback never trigger another plan review. A new explicit user request for plan review invokes it normally; the planner never infers a second review.
...
Present artifact paths (`plan.md`, `execute.md`, `tasks.json`, `goal-prompts.md`, `reviews/…`), applied review changes, skipped items, and any blocked scope-change recommendation. Count the final parents, subtasks, and dependency waves, then render the compact **Execution guidance** block required by `references/estimation-guidance.md` immediately before the final approval sentence. Guidance is informational and never delays or blocks the gate; render its unavailable state when no valid analog exists. If scope change is required, say *"This requires a scope change; I did not apply it"* and ask whether to reopen scope or keep the scope-preserving plan. Else prompt: *"Final reviewed plan/tasks and portable goal prompts are ready. Reply `Approved` to proceed to execution, or give final feedback."* Wait.
- **Feedback preserving scope** → the primary directly applies the smallest `plan.md` edit and regenerates affected task artifacts when approach/verification/deps/sequencing/references change; for task-translation-only corrections, the primary edits `tasks.json` plus affected `execute.md` rows. Run deterministic parse/reference/post-checks and regenerate `goal-prompts.md`, but **do not re-run `plan_review` or `task_review`**. Report that the edit happened after the single review round.
