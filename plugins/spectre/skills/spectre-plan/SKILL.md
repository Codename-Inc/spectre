...
- **One-review hard stop:** each planning run gets at most one plan-review round and, for COMPREHENSIVE, at most one task-review round. Once a review round has started, later artifact edits, final-gate feedback, small discussion points, stale hashes, helper output, or safety uncertainty **MUST NOT** trigger another semantic review. Only the user's latest instruction explicitly asking to review again may authorize `--review-again`; the planner must never infer or add that override itself.
...
**DONE when:** tier was classified and logged; the tier's required skills ran (not described); required artifacts exist for the tier; `execute.md` has Document Manifest / Task Detail Source / Execution Summary / Wave Plan / Parent Task Index / Slicing Rules; referenced `tasks.json` parses; for every non-MICRO tier, `goal-prompts.md` contains portable-strict structured + compact prompts generated after the tier's final task-artifact step; for STANDARD/COMPREHENSIVE, the Initial Design Proposal Gate was approved and recorded as `## Selected Design`, exactly one independent plan-review round produced a saved report before task generation, scope-safe plan findings are integrated, the Final Gate was presented, and the post-tasks re-check passed/was surfaced; for COMPREHENSIVE, exactly one task-review round produced a saved report after task generation and scope-safe task findings are integrated before goal generation; canonical scope is unchanged (or a scope gate was raised). A review round that failed or was interrupted is surfaced as such and is not automatically restarted.
...
- **Feedback preserving scope** → if it changes approach/verification/deps/sequencing/references, apply the smallest `plan.md` edit and regenerate affected task artifacts; if it only corrects task translation, edit `tasks.json` plus affected `execute.md` rows. Run deterministic parse/reference/post-checks and regenerate `goal-prompts.md`, but **do not re-run `plan_review` or `task_review`**. Report that the edit happened after the single review round.
- **Explicit review-again request** → only when the user's latest instruction expressly asks for another plan or task review, invoke that specific review skill with `--review-again`, record the authorization in its attempt ledger, and refresh downstream artifacts as needed. Generic feedback, a scope-preserving edit, or a request to continue is not authorization.
