---
name: "spectre-scope"
description: "Scope a feature or improvement into explicit IN / OUT / ANTI-SCOPE boundaries before planning or code — grounds a hypothesis in repo reality, resolves blocking questions, and writes scope.md. Trigger for new or fuzzy work, re-scoping, or the one-confirmation boundary pass requested by spectre-align-and-deliver. Do NOT trigger for technical design/research (spectre-plan) or standalone bug diagnosis (spectre-fix)."
user-invocable: true
---
...
- `DELIVERY_ALIGNMENT=one-confirmation` + `--orchestrated` → preserve the canonical scope contract but use one consolidated user response; return `NEEDS_FULL_SCOPE` instead of iterating when one pass is unsafe.
...
- **Ground once.** Start with exactly **one** fast lookup to anchor the hypothesis in repo reality — a single `@spectre:finder` query, or one `grep`/`glob`; skip it if slow. In default interactive mode, needing broader grounding exceeds this fast scope pass. One-confirmation mode explicitly exempts the targeted read-only pre-research below from the one-lookup limit.
- **Default interactive mode.** Lead with a grounded hypothesis (problem, who it affects, proposed feature name/root, IN / OUT / ANTI-SCOPE) and 5–8 questions tagged **(blocking)** / *(optional)*. Iterate boundaries (IN / OUT / ANTI-SCOPE / Unsure) until confirmed, then clarify remaining ambiguity with `AskUserQuestion` (≤4 at a time). No clarification files.
- **One-confirmation delivery mode.** When `DELIVERY_ALIGNMENT=one-confirmation`:
  1. If the fast lookup is insufficient, run targeted read-only `@spectre:finder`, `@spectre:analyst`, and `@spectre:patterns` pre-research before prompting.
  2. Present one consolidated hypothesis with the proposed feature name/root, IN / OUT / ANTI-SCOPE, and only blocking questions; record optional unknowns as load-bearing assumptions with *"if this is false, …"* consequences.
  3. **WAIT exactly once.** Confirmation or coherent corrections authorize writing `scope.md`. If a cohesive boundary still cannot be formed, write nothing and return `NEEDS_FULL_SCOPE` with the unresolved tensions; do not iterate or guess in this mode.
...
With `DELIVERY_ALIGNMENT=one-confirmation --orchestrated`, return either `SCOPE_FILE={actual path}` plus final boundaries/assumptions, or `NEEDS_FULL_SCOPE` plus unresolved tensions. The parent owns routing and user-facing Next Steps.

For standalone usage, use the routing below.
...
- In default interactive mode, grounding needs more than one lookup, or the request spans several unknowns → this exceeds a scope; gather context more fully before proceeding. In one-confirmation mode, use its bounded pre-research and return `NEEDS_FULL_SCOPE` only if a cohesive boundary still cannot be formed.
...
- One-confirmation mode cannot form cohesive boundaries after its single response → return `NEEDS_FULL_SCOPE`; do not continue to implementation.
