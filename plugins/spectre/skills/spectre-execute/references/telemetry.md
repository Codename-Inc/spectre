...
```bash
spectre-workflow agent measure --run-id "$RUN_ID" --actor-id "$PRIMARY_ACTOR_ID" --worker-actor-id "$WORKER_ACTOR_ID" --child-agent-id "$CHILD_AGENT_ID" --measurement-snapshot '<dispatch returned JSON>' --project-dir "$PROJECT_ROOT" --json
```
