...
```bash
spectre-workflow run start --source "$TASKS_JSON" --origin "$ORIGIN" --owner "$FINALIZATION_OWNER" --project-dir "$PROJECT_ROOT" --json
```
...
```bash
spectre-workflow run start --source "$TASKS_JSON" --origin "$ORIGIN" --project-dir "$PROJECT_ROOT" --json
