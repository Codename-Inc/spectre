...
description: "Build a planned feature from either a compact execute.md/tasks.json work source or an explicit readable plan, dispatching parallel task waves through risk-adaptive deterministic gates, lightweight sentinel review, one risk-triggered adversarial checkpoint, and an optional parent-owned finalization boundary. Trigger after structured task artifacts or an executable plan exist, or to resume a partial run. Do NOT trigger for scoping/planning, unplanned bug-fixing, or dead-code cleanup (spectre-prune)."
user-invocable: true
---

# execute

Execute work in parallel waves without loading or generating an exhaustive task graph in primary-agent context. Resolve structured task artifacts before generic plan input, verify each wave against the selected deterministic profile, adapt only for source-backed compliance, run one early adversarial checkpoint for material risk, and either own final cumulative verification/review or return an explicit implementation-ready handoff to a parent that owns those final gates.
...
- Optional `--verification-profile bounded|strict`. Default `strict`. `bounded` uses primary-owned affected lint/typecheck/build plus native related tests per wave, promotes permanently to `strict` when risk or test-topology evidence requires it, and never weakens the final complete-suite requirement.
- Optional `--finalization-owner parent`. It is valid only with `--orchestrated`; otherwise stop. Without it, `FINALIZATION_OWNER=self` and execute preserves its full final suite/review/test-guide contract. With it, execute returns `IMPLEMENTATION_READY` after wave completion and the named parent must own post-clean/rebase complete-suite verification, cumulative review, and proof.
...
- Record `VERIFICATION_PROFILE` (`bounded|strict`), any promotion reason, and `FINALIZATION_OWNER` (`self|parent`) in the structured execution summary or plan-direct **Runtime Status**. Plain `--orchestrated` changes handoff only and never implies either non-default value.
...
- Before the first dev dispatch, create or resume `execution_state.md` with `Feature: <feature-name>` and `Feature Root: .spectre/features/<feature-name>` immediately below its title, followed by exactly these seven sections:
  ...
  5. **Wave History** — completed assignments, commits, changed files, verification profile/promotions, test-suite baseline ledger and deterministic checks/results, sentinel/checkpoint classification/result, compact repair ledger (`invariant_family · finding_fingerprint · attempt · route · test topology · disposition`), and E2E completeness signal.
...
- Under `strict`, before the first wave that can change an affected package/workspace, the primary resolves its authoritative complete-suite command from repository manifests. A verifiable successful CI check for the exact baseline SHA may establish a green baseline only when it names the same command and compatible environment; persist its check identity and URL. Otherwise run that exact command at the recorded baseline Git SHA in a clean worktree. Persist the command, source (`ci|local`), exit status, and exact failing test identities plus stable failure signatures in the structured execute index's execution summary or plan-direct **Wave History**. A red/unknown CI result never substitutes for a local baseline. Never establish a baseline from a tree containing feature changes. A newly discovered test root gets the same baseline treatment before its first modifying strict wave.
- Under `bounded`, do not run a complete-suite baseline. Before every wave gate, prove the wave remains confined to one package/workspace, has reliable native related-test selection, and touches none of: auth, permissions, payments, PII, migrations, public APIs/contracts, secrets, network input, destructive actions, data correctness, persistent state, external side effects, shared infrastructure/build/test tooling, or a cross-package lifecycle/trust boundary. If any condition fails or is uncertain, record the reason, promote permanently to `strict`, establish strict baselines from the recorded baseline SHA, and continue the same wave.
...
3. **Per-wave verification gate** (order is load-bearing):
   - **3a — Deterministic profile gate:** the primary—not an implementation-agent report—resolves commands from `package.json`/`pyproject.toml`/`Cargo.toml`/`Makefile` and repository-native test configuration.
     - `bounded`: independently run affected-package lint/typecheck/build plus the repository-native related/focused tests selected from the wave diff. Implementation-agent test reports are supporting evidence only. An unavailable or ambiguous related-test command promotes to `strict`; it never becomes a green bounded result.
     - `strict`: run lint/typecheck/build plus the authoritative complete suite for every package/workspace affected by the wave. Focused/related tests remain required TDD evidence but never substitute for this gate. Compare each suite with its persisted baseline using the exact same command and compatible environment: a green baseline must remain green; a red baseline permits only the exact recorded failing test identities with matching stable failure signatures. Count-only comparison is forbidden. Any new, changed, or unclassified failure keeps the wave red; an unavailable required suite is a blocker, never a green result.
     - Fix until the active profile is green. Never invoke reviewers while deterministic checks fail.
   ...
   - **3d — Finding disposition + invariant-family repair:** Before repairing any CRITICAL/HIGH finding from a sentinel, checkpoint, or final review, the primary classifies it against the authoritative source as `defect` (the completed slice violates a current requirement), `scheduled` (the exact missing behavior is owned by explicit pending source work and is not required by the completed slice), or `scope-change` (repair would add/change requirements). Record the evidence-backed disposition. `scheduled` consumes no attempt; `scope-change` uses the normal user-authority gate. Track repair attempts by `invariant_family`, while `finding_fingerprint` preserves exact manifestation identity; distinct invariant families have independent budgets and there is no global sentinel/fix cap. Attempt 1 uses one focused `@spectre:dev` repair. A second manifestation of the same invariant family—regardless of changed symbol or symptom—forces the primary to revalidate and own attempt 2, first producing and observing a RED regression at the invariant's actual level across the demonstrated transition chain (for example write -> reload -> reconcile), then changing both diagnosis and route: implement directly or use a clean-context, high-effort opposing runtime; never replay the same prompt, agent, or approach. If the invariant family survives attempt 2, end retries and promote it to source-backed Adapt work with revised implementation shape/dependencies. Growth >25% or test-file changes >0.5x implementation-file changes may trigger Adapt sooner. Continue through 3a–3c; halt only for scope change or user authority.
...
- `FINALIZATION_OWNER=parent`: validate that every wave passed its active deterministic profile, no required CRITICAL/HIGH remains, and the task/workstream completion projection is satisfied. Do not rerun complete suites, dispatch final `spectre-code_review`, or create a test guide. Return `IMPLEMENTATION_READY` with the cumulative diff/base, commits/files, affected package/workspace and test-root manifest, exact commands/results, profile/promotions, sentinel/checkpoint evidence, repair ledger, source requirements paths/slices, and unresolved Medium/Low findings. The parent must use these sources without implementer rationale and owns all final candidate gates.
- `FINALIZATION_OWNER=self`: rerun every registered affected-scope suite; under `strict`, compare it with its baseline. When the repository exposes a single authoritative root suite, run it once and do not duplicate an identical affected-scope command at the same HEAD. Only after those deterministic checks pass, run the expensive review once over the cumulative feature diff:
  ...
  - Read the saved report. CRITICAL/HIGH defects re-enter Reflect/Adapt under the same disposition and invariant-family repair policy; Medium/Low findings are summarized but do not block completion unless the user asks. After a repair, rerun deterministic suites and only the affected review gate; any cumulative change still requires a fresh final review.
...
- `FINALIZATION_OWNER=parent`: an `IMPLEMENTATION_READY` handoff with the manifest required above; no claim that final complete-suite, review, proof, or candidate verification has run.
- Completion summary: tasks done · waves · verification profile/promotions · affected-suite baseline/delta or bounded-command status · sentinel review counts (`skip`/`wiring`/`risk-checkpoint`) · invariant-family repair counts · finalization owner/status · final review status when self-owned · requirement-delivery coverage status when self-owned · test-guide path when self-owned · Task Evolution Summary · E2E Gaps Addressed · Unresolved Findings.
...
- **IMPLEMENTATION_READY when parent-owned:** the selected mode's completion rule holds; every wave passed its active deterministic profile plus any required sentinel/checkpoint review; no required CRITICAL/HIGH remains; and the complete parent handoff manifest is returned.
- **DONE when self-owned:** the selected mode's completion rule holds; every wave passed its active deterministic profile plus any required sentinel/checkpoint review; final complete suites pass; final adversarial review's exhaustive delivery coverage is clean or gaps fixed/accepted; test guide is written; and the summary is returned.

## Handoff

Report the summary inline (counts, verification profile/promotions, fix-loop iterations, finalization owner/status, unresolved findings, and test-guide path when self-owned).
...
- `--finalization-owner parent` without `--orchestrated`, an unknown verification profile, or a parent that cannot accept the required implementation-ready manifest → stop before implementation.
- A subagent prompt slice contains unrelated parent task ids → fix the slice before dispatch.
- Never request approval solely because of repair count, reviewer count, diff growth, or a recurring invariant family. Checkpoint/final-review findings use the same disposition and invariant-family repair policy. Escalate only for scope change or user authority.
- Deterministic suite gate cannot pass → fix before review; never advance a red wave.
