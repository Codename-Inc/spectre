...
Build the selected source without an exhaustive graph in primary context. Maximize dependency-safe parallelism—dispatchable work never idles—while preserving authority, checkout, phase gates, repairs, and finalization.
...
1. **Resolve and resume.** Load only the selected source and required state. In structured mode initialize telemetry and start the execute/phase/wave boundaries per its reference. A direct-marked plan (`Execution Mode: direct`) must carry its seven spine sections; a missing spine escalates to `/spectre:plan`. Legacy unmarked plans start without a quality/completeness gate. Preserve source order unless dependency evidence records a reordering. Detail only the current dependency-ready frontier.
2. **Batch and dispatch.** Maximize parallel delivery: the moment a parent's dependencies are all accepted, dispatch it—ready work never waits on an incomplete upstream wave or phase. Waves are batching hints and phases are review-routing boundaries; only unaccepted dependencies block dispatch. At most three sequential structured parents per assignment or bounded plan-direct workstreams. The primary may implement one sequential workstream; use `@spectre:dev` only for independent parallel work or context protection. For a `risk`-annotated parent, explicitly request the high-capability model override on dispatch (Claude `claude-opus-5`; Codex `gpt-5.6-sol`); the agent default otherwise handles it. Brief exact assignment, scope/prior-wave/plan context only—never the tasks file or unrelated ids; name a risky parent's risk and threatened invariant, and give parents converging on a shared contract the same verbatim contract slice. Structured dispatch includes the exact `<workflow_telemetry>` assignment for every parent/subtask. Require `@skill-spectre:spectre-tdd`, focused checks, assignment commits, compressed insights, and E2E `Complete|Gap|Adaptation`.
3. **Gate accepted batches.** Before any reviewer, the primary independently runs affected lint/typecheck/build and native focused tests selected from the returned batch diff plus real consumer/integration/lifecycle boundaries; gate completed batches while other assignments remain in flight, and let early-dispatched parents ride the next covering gate. Never run a repository baseline/root suite. Record the verification gate without raw output. Attribute failures and follow `references/repair-policy.md`; implementation reports are supporting evidence only.
...
- After that authoritative terminal status, follow the telemetry reference to emit one non-authoritative `plan.execution_outcome`; join failure only degrades telemetry.
