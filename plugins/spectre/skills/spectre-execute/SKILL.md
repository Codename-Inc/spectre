...
2. **Batch and dispatch.** Choose dependency-ready work: at most three sequential structured parents per batch or bounded plan-direct workstreams. The primary may implement one sequential workstream; use `@spectre:dev` only for independent parallel work or context protection. Brief exact assignment, scope/prior-wave/plan context only—never the tasks file or unrelated ids. Structured dispatch includes the exact `<workflow_telemetry>` assignment for every parent/subtask. Require `@skill-spectre:spectre-tdd`, focused checks, assignment commits, compressed insights, and E2E `Complete|Gap|Adaptation`.
...
Return counts, affected/final coverage, intermediate-review routing/final-review status, finding dispositions, owner/status, self-owned proof status, unresolved findings, artifact paths, and structured `RUN_ID` + telemetry `complete|degraded`. Parent-owned runs return `IMPLEMENTATION_READY`/`ACCEPTANCE_PENDING` without user-facing next steps. Otherwise render one recommended next step from unresolved high findings → `/spectre:fix`, concrete coverage risk → `/spectre:test`, else `/spectre:clean`.
