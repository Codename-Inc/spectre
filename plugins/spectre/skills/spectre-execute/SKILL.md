...
description: "Execute structured tasks or a readable plan in dependency-safe waves with affected verification, phase-complete review, one final adversarial review, and end-only proof. Use after planning or to resume; not for planning, unplanned fixes, or pruning."
user-invocable: true
---
...
- DONE: selected work/adaptations `done|skipped`; current affected verification; one review per completed phase; every finding dispositioned after at most one repair pass; no stale/uncovered final surface; and self-owned final review + end-only proof when applicable.
...
2. **Batch and dispatch.** Choose dependency-ready work: at most three sequential structured parents per batch or bounded plan-direct workstreams. The primary may implement one sequential workstream; use `@spectre:dev` only for independent parallel work or context protection. Brief exact assignment, scope/prior-wave/plan context only—never the tasks file or unrelated ids. Require `@skill-spectre:spectre-tdd`, focused checks, assignment commits, compressed insights, and E2E `Complete|Gap|Adaptation`.
...
5. **Review phase milestones.** A phase is reviewable only when all its source-owned tasks/workstreams and required adaptations are `done|skipped` with current affected verification. After each wave, send all newly completed phases in one lightweight `@spectre:reviewer` call, with an independent verdict per phase. Never review a partial phase or review every wave.
   - Inputs: phase diff/commits/files, scope, and verbatim requirement/AC/context slices—no dev rationale, pending phases, or unrelated files.
...
- `parent`: require phase gates + finding dispositions; return cumulative diff/base, commits/files, package/test roots, verification coverage, phase-review evidence, repair/routing ledger, requirement sources/slices, unresolved findings, and `ACCEPTANCE_PENDING`.
