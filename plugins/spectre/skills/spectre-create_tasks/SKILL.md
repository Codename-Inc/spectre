...
- Research only if plan/context do not name target files/patterns: dispatch `@spectre:finder`, `@spectre:analyst`, `@spectre:patterns`. Research agents return evidence only and never write planning artifacts; the primary folds returns into `task_context.md` `## Technical Research`. Skip for LIGHT or clear single-component scope.
