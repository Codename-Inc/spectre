...
4. **Draft lifecycle:** pending writes a grounded body with local verification `RUNNING`, pushes, then creates the draft and returns URL/body. Final-update rechecks its tuple/clean candidate and draft, replaces only Testing in the returned body using `FINAL_VERIFICATION_SUMMARY`, then `gh pr edit`s that draft; never mark ready. If repairs changed the tuple, refresh candidate-sensitive claims under the same freshness, grounding, and secret gates in a pending pass before pushing/updating; only its later final-update changes Testing.
