 ...
 *
 * Registers a caspar learning and manages the project-level recall skill.
 *
 * Responsibilities:
 * 1. Create/update registry at .claude/skills/caspar-recall/references/registry.toon
 * 2. Read recall-template.md from plugin
 * 3. Generate .claude/skills/caspar-recall/SKILL.md with embedded registry
 *
 * Usage:
...
function resolvePluginSkillPath(pluginRoot, skillName, ...parts) {
  ...
  const legacyBareName = skillName.startsWith('caspar-') ? skillName.slice('caspar-'.length) : null;
  if (legacyBareName) {
    candidates.push(
...
function main() {
  ...
  // New paths: registry lives inside caspar-recall skill
  const recallDir = path.join(skillBaseDir(projectRoot), 'caspar-recall');
  ...
  const templatePath = resolvePluginSkillPath(pluginRoot, 'caspar-learn', 'references', 'recall-template.md');
