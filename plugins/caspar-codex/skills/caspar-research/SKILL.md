...
- Agents (read-only, run in parallel): `@finder` (where code lives), `@analyst` (how it works), `@patterns` (similar implementations), `@web-research` (3rd-party docs — instruct it to return LINKS). Context7 MCP only if the user explicitly asks for library docs.
